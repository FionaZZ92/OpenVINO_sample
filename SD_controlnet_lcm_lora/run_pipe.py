...
from diffusers import StableDiffusionControlNetPipeline, ControlNetModel, LCMScheduler, DiffusionPipeline
import torch
import gc
import numpy as np
import argparse
...
def parse_args() -> argparse.Namespace:
    ...
    args.add_argument('-m', '--model_weights', type = str, default = "", required = True,
                      help='Specify path of Stable Diffusion Model path')
    args.add_argument('-ctrl', '--controlnet', type = str, default = "", required = False,
            ...
            for iter in range(len(state_dict)):
                ...
                for key in state_dict[iter]:
                    ...
                    if len(state_dict[iter][pair_keys[0]].shape) == 4:
                        ...
                        lora_weights = alpha_list[iter] * torch.matmul(weight_up, weight_down)
                        lora_dict.update(value=lora_weights)
    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = self.unet([latent_model_input, t, text_embeddings, *down_and_mid_blok_samples],share_inputs=True, share_outputs=True)[self.unet_out] #[self.unet_out]
...
controlnet = ControlNetModel.from_pretrained(args.controlnet, torch_dtype=torch.float32).cpu()
pipe = StableDiffusionControlNetPipeline.from_pretrained(args.model_weights,controlnet=controlnet)

#tokenizer = CLIPTokenizer.from_pretrained('clip-vit-large-patch14')
tokenizer = AutoTokenizer.from_pretrained(args.model_weights, subfolder="tokenizer")
#scheduler = UniPCMultistepScheduler.from_config(pipe.scheduler.config)
if not SCHEDULER_PATH.exists():
    scheduler = LCMScheduler.from_pretrained(args.model_weights, subfolder="scheduler")
...
CONTROLNET_OV_PATH = "./model/controlnet-canny.xml"
TEXT_ENCODER_OV_PATH = "./model/text_encoder.xml"
UNET_OV_PATH = "./model/unet_controlnet.xml"
VAE_DECODER_OV_PATH = "./model/vae_decoder.xml"
...
negative_prompt = ["3d, cartoon, lowres, bad anatomy, bad hands, text, error"]
